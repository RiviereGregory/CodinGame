	...
	void run() {
		...
		while (true) {
			...
			for (Entity robot : board.myTeam.robots) {
				int y = getRandomNumberInRange(0,board.height);
				int x = getRandomNumberInRange(1, board.width);
				Coord posA = new Coord(x,y);
				Coord posMy = new Coord(robot.pos.x,robot.pos.y);
				//Coord posDig = new Coord(robot.pos.y,robot.pos.y);
				Cell cell = board.getCell(posMy);
				if (robot.item == EntityType.AMADEUSIUM) {
					Coord pos = new Coord(0,robot.pos.y);
					robot.action = Action.move(pos);
				} else if (!cell.known || (cell.hole && cell.ore>0)){
					robot.action = Action.dig(posMy);
				} else {
					robot.action = Action.move(posA);
				}
